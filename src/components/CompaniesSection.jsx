  ...
  return (
    <section
      className={`relative py-24 bg-gradient-to-br overflow-hidden ${cx(
        'from-white via-primary-50/30 to-purple-50/30',
        'from-gray-800 via-gray-900 to-purple-950/20'
      )}`}
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className={cx('text-gray-600', 'text-gray-300')}>Loading companies...</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <Link
              ...
              className={`group relative backdrop-blur-xl rounded-3xl p-8 text-center shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer border-2 transform hover:scale-105 hover:-translate-y-2 block ${cx(
            ...
            >
              ...
              <div className="relative z-10">
                <div className={`mb-6 p-4 bg-gradient-to-br ${getGradient(company.industry)} rounded-2xl inline-block transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg`}>
                  <div className={`backdrop-blur-sm rounded-xl p-4 w-20 h-20 flex items-center justify-center ${cx('bg-white/90', 'bg-gray-800/90')}`}>
                    <img
                      src={company.logo}
                    ...
                    />
                    <div className={`text-3xl font-bold hidden w-full h-full items-center justify-center ${cx('text-primary-600', 'text-primary-400')}`}>
                      {company.name.charAt(0)}
                    </div>
                  </div>
                </div>

                <h3 className={`text-xl font-bold mb-3 group-hover:bg-gradient-to-r group-hover:from-primary-600 group-hover:to-purple-600 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300 ${cx('text-gray-900', 'text-white')}`}>
                  {company.name}
                </h3>

                {company.employees && (
                  <div className={`mb-2 text-sm ${cx('text-gray-600', 'text-gray-400')}`}>
                    {company.employees.toLocaleString()} employees
                  </div>
                )}

                <div className={`mb-4 font-semibold ${cx('text-gray-600', 'text-gray-400')}`}>
                ...
                <div className="flex items-center justify-center mb-4">
                      ...
                      <svg
                        ...
                        className={`w-5 h-5 transition-all duration-300 ${
                          i < Math.floor(company.rating)
                            ...
                            : cx('fill-gray-300', 'fill-gray-600')
                        }`}
                        viewBox="0 0 24 24"
                      >
                  ...
                  <span className={`font-bold ${cx('text-gray-700', 'text-gray-300')}`}>
                    {company.rating}
                  </span>
                </div>
